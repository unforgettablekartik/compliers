...
export function AccordionTrigger({
  ...
  return (
    <button
      ...
      {...props}
    >
      <span className="flex-1 pr-4 text-base font-normal text-gray-900">{children}</span>
      <span className="text-2xl font-light text-gray-400 leading-none select-none flex-shrink-0 ml-auto" aria-hidden="true">
        {isOpen ? '−' : '+'}
      </span>
    </button>
  );
...
export function AccordionContent({
  ...
  return isOpen ? (
    <div className={`pb-6 pt-0 pr-0 pl-0 bg-transparent text-gray-600 text-sm leading-relaxed ${className}`} {...props}>{children}</div>
  ) : null;
}
